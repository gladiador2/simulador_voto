...
function click_candidato(evento) {
    ...
    // Restriccion: en INTENDENTE solo se permite seleccionar a
    // "DR. AMILCAR MIÑO" (lista 1, id_umv 411). Cualquier otra opcion
    // (otro candidato o voto en blanco) muestra el aviso y no se selecciona.
    if (categoria.codigo === "INT" && codigo !== "411") {
        bindear_botones();
        mostrarModalNoHabilitado();
        return;
    }

    // Restriccion: en JUNTA MUNICIPAL solo se permite elegir la LISTA 1
    // (id_umv 7419, la que representa a esa lista en esta pantalla).
    // El detalle del candidato dentro de la lista se restringe aparte en
    // click_preferencia_candidato().
    if (categoria.codigo === "JUN" && codigo !== "7419") {
        bindear_botones();
        mostrarModalNoHabilitado();
        return;
    }
...
function click_preferencia_candidato(evento) {
    ...
    // Restriccion: en JUNTA MUNICIPAL solo se permite elegir la Opción 7,
    // "VENANCIO CABRERA" (id_umv 7425).
    if (categoria.codigo === "JUN" && codigo !== "7425") {
        mostrarModalNoHabilitado();
        return;
    }
...
/**
 * Muestra el aviso de que la opcion elegida no esta habilitada para esta
 * simulacion (solo se puede seleccionar al candidato a Intendente de la
 * lista 1, Dr. Amilcar Miño).
 */
function mostrarModalNoHabilitado() {
    if (document.getElementById("modalNoHabilitado")) {
        return;
    }
    var modal = document.createElement("div");
    modal.id = "modalNoHabilitado";
    modal.className = "modal-overlay";
    modal.innerHTML =
        '<div class="modal-content">' +
        '<div class="modal-icon">⚠️</div>' +
        "<h2>Seleccione otra opción</h2>" +
        '<button type="button" class="btn-aceptar">Aceptar</button>' +
        "</div>";
    modal
        .querySelector(".btn-aceptar")
        .addEventListener("click", cerrarModalNoHabilitado);
    document.body.appendChild(modal);
}

/**
 * Cierra el aviso de opcion no habilitada.
 */
function cerrarModalNoHabilitado() {
    var modal = document.getElementById("modalNoHabilitado");
    if (modal) {
        modal.remove();
    }
}
